...
print({'$lager_record_newline', Name, Fields}, Max, Options) ->
    Leader = "#" ++ atom_to_list(Name) ++ "{\n",
    {RC, Len} = record_fields_nl(Fields, Max - length(Leader) + 1, dec_depth(Options), 2),
    {[Leader, RC, "\n}"], Len + length(Leader) + 1};

print({'$lager_map_newline', Map}, Max, Options) ->
    {MapBody, Len} = map_body_nl(Map, Max - 3, dec_depth(Options)),
    {["#{\n", MapBody, "}\n"], Len + 5};
...
map_body_nl(Map, Max, #print_options{depth=Depth}) when Max < 4; Depth =:= 0 ->
    case erlang:map_size(Map) of
        0 -> {[], 0};
        _ -> {"...", 3}
    end;
map_body_nl(Map, Max, Options) ->
    case maps:to_list(Map) of
        [] ->
            {[], 0};
        [{Key, Value} | Rest] ->
            ...
            {ValueStr, ValueLen} = print(Value, Max - DiffLen, Options),
            DiffLen2 = DiffLen + ValueLen,
            {Final, FLen} = map_bodyc_nl(Rest, Max - DiffLen2, dec_depth(Options)),
            {["  ", KeyStr, " => ", ValueStr, "\n" | Final], DiffLen2 + FLen}
    end.

map_bodyc_nl([], _Max, _Options) ->
    {[], 0};
map_bodyc_nl(_Rest, Max,#print_options{depth=Depth}) when Max < 5; Depth =:= 0 ->
    {",...", 4};
map_bodyc_nl([{Key, Value} | Rest], Max, Options) ->
    {KeyStr, KeyLen} = print(Key, Max - 5, Options),
    DiffLen = KeyLen + 5,
    {ValueStr, ValueLen} = print(Value, Max - DiffLen, Options),
    DiffLen2 = DiffLen + ValueLen,
...
record_fields_nl([], _, _, _) ->
    {"", 0};
record_fields_nl(_, Max, #print_options{depth=D}, _) when Max < 4; D == 0 ->
    {"...", 3};
record_fields_nl([{Field, Value}|T], Max, Options, Tabs) ->
    {ExtraChars, Terminator} = case T of
        [] ->
            {0+2+3, []};
        _ ->
    ...
    {FieldStr, FieldLen} = print(Field, Max - ExtraChars, Options),
    {ValueStr, ValueLen} = print(Value, Max - (FieldLen + ExtraChars), Options),
    {Final, FLen} = record_fields_nl(T, Max - (FieldLen + ValueLen + ExtraChars), dec_depth(Options), Tabs),
    {["  "++FieldStr++" = "++ValueStr++Terminator|Final], FLen + FieldLen + ValueLen + ExtraChars}.
