...
-module(lager_file_backend_node).
...
init(LogFileConfig) when is_list(LogFileConfig) ->
    case validate_logfile_proplist(LogFileConfig) of
        ...
        Config ->
            ...
            Name = lager_util:expand_path(parse_rel_name(RelName)),
...
handle_event({log, Message},
    ...
    {_, Mask} = L,
    WriteF = lager_msg:severity_as_int(Message) * 2 - 1 =:= Mask,
    case WriteF andalso lager_util:is_loggable(Message,L,{lager_file_backend, Name}) of
        true ->
...
parse_rel_name(RelName) ->
    ...
    ExtraStr = io_lib:format("_~w.log", [NodeId]),
    string:replace(RelName, ".log", ExtraStr).