...
-export([start/0,
        ...
        do_log/9, do_log/10, do_log_unsafe/10, pr_newline/2, pr/2, pr/3, pr_stacktrace/1, pr_stacktrace/2]).
...
pr_newline(Record, Module) when is_tuple(Record), is_atom(element(1, Record)) ->
    pr(Record, Module, [newline]);
pr_newline(List, Module) when is_list(List) ->
    pr(List, Module, [newline]);
pr_newline(Map, _) when is_map(Map) ->
    {'$lager_map_newline', Map};
pr_newline(Record, _) ->
    Record.
    ...
    try 
        case is_record_known(Record, Module) of
            ...
            {RecordName, RecordFields} ->
                ...
                case lists:member(newline, Options) of
                true ->
                    {'$lager_record_newline', RecordName,
                        zip(RecordFields, tl(tuple_to_list(Record)), Module, Options, [])};
                _ ->
                    {'$lager_record', RecordName,
                        zip(RecordFields, tl(tuple_to_list(Record)), Module, Options, [])}
...
zip([ti_itxbr|RecordFields], [_|Record], Module, Options, ToReturn) ->
    zip(RecordFields, Record, Module, Options, ToReturn);
zip([to_aejnv|RecordFields], [_|Record], Module, Options, ToReturn) ->
    zip(RecordFields, Record, Module, Options, ToReturn);
