...
init([]) ->
    ...
    T5CrashLog = decide_t5_crash_log(application:get_env(lager, t5_crash_log, false)),

    {ok, {{one_for_one, 10, 60},
          Children ++ CrashLog ++ T5CrashLog
         }}.
...
decide_t5_crash_log(undefined) ->
    [];
decide_t5_crash_log(false) ->
    [];
decide_t5_crash_log(File) ->
    MaxBytes = validate_positive(application:get_env(lager, t5_crash_log_msg_size), 65536),
    RotationSize = validate_positive(application:get_env(lager, t5_crash_log_size), 0),
    RotationCount = validate_positive(application:get_env(lager, t5_crash_log_count), 0),

    RotationDate = determine_rotation_date(application:get_env(lager, t5_crash_log_date)),
    RotationMod = determine_rotator_mod(application:get_env(lager, t5_crash_log_rotator), lager_rotator_default),

    [{lager_t5_crash_log,
        {lager_t5_crash_log, start_link, [
            File,
            MaxBytes,
            RotationSize,
            RotationDate,
